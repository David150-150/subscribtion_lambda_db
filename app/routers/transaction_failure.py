...
def create_transaction_failure(
    ...
    return crud.transaction_failures.create_transaction_failure(
...
def get_transaction_failure(transaction_failure_id: int, db: Session = Depends(get_db)):
    transaction_failure = crud.transaction_failures.read_transaction_failure(
        db, transaction_failure_id
    )
    if not transaction_failure:
        raise HTTPException(
            status_code=404, detail=f"ID:{transaction_failure_id}, not found"
        )
