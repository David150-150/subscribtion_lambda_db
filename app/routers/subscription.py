...
def read_subscription(subscription_id: int, db: Session = Depends(get_db)):
    ...
    if not subscription:
        raise HTTPException(status_code=404, detail=f"The subscription id:{subscription_id} does not exist")
    return subscription

@router.put("/{subscription_id}", response_model = schemas.SubscriptionOut)
def update_subscription(subscription_id: int, subscription_data: schemas.SubscriptionCreate, db: Session = Depends(get_db)):
