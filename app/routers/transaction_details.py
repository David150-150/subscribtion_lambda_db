...
def create_transaction_details(
    transaction_details: schemas.Transaction_Detail_Base, db: Session = Depends(get_db)
):
...
def get_transaction_detail(transaction_detail_id: int, db: Session = Depends(get_db)):
    transaction_detail = crud.transaction_details.read_transaction_detail(
        db, transaction_detail_id
    )
    if not transaction_detail:
        raise HTTPException(
            status_code=404, detail=f"ID:{transaction_detail_id}, not found"
        )
